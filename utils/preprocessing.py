...
def augment_data(data:pd.DataFrame, image_path:str='data/SDD/train', image_file:str='reference.jpg', seg_mask:bool=False):
	'''
	Perform data augmentation
	:param data: Pandas df, needs x,y,metaId(different augment),sceneId columns
	...
	:return:
	'''
	image_dict = {} # key and value as {sceneId: PIL image}
	for scene in data.sceneId.unique():
		im_path = os.path.join(image_path, scene, image_file)
		im = cv2.imread(im_path, int(not seg_mask))  # 1(default):color, 0:grayscale
		image_dict[scene] = im

	data_ = data.copy()  # data without rotation, used so rotated data can be appended to original df
	k2rot_list = ['_rot90', '_rot180', '_rot270'] # k*90
	for k, r in enumerate(k2rot_list):
		...
		for scene in data_.sceneId.unique():
			...
			image_dict[scene + r] = im

			data_rot['sceneId'] = scene + r
	...
	for scene in data.sceneId.unique():
		...
		image_dict[scene + '_fliplr'] = im_flip
...
def create_images_dict(data, image_path, image_file='reference.jpg') -> dict:
	images_dict = {}
	for scene in data.sceneId.unique():
		im = cv2.imread(os.path.join(image_path, scene, image_file))
		images_dict[scene] = im
	return images_dict
