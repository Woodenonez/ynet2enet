	...
	with torch.no_grad():
		...
		for trajectory, meta, scene in val_loader:
			...
			for i in range(0, len(trajectory), batch_size):
				print(f'\r[VAL] Scene: {cnt}/{len(val_loader)}; Batch: {i+1}/{len(trajectory)}   ', end='')
				...
				for waypoint in waypoint_samples:
					...
					waypoint_map_tmp = waypoint_map.clone()

					pred_traj_map = waypoint_map
					for _ in range(1, gt_future.shape[1]):
						waypoint_maps_downsampled = [nn.AvgPool2d(kernel_size=2 ** i, stride=2 ** i)(waypoint_map_tmp) for i in range(1, len(features))]
						waypoint_maps_downsampled = [waypoint_map_tmp] + waypoint_maps_downsampled
						...
						waypoint_map_tmp = pred_trajwp_map
...
def visualize(model, val_loader, val_images, num_goals, num_traj, obs_len, batch_size, device, input_template, waypoints, resize, temperature, use_TTST=False, use_CWS=False, rel_thresh=0.002, CWS_params=None, dataset_name=None, homo_mat=None, mode='val'):
	"""
	...
	assert(dataset_name=='sdd'), ("Dataset must be SDD.")
	...
	with torch.no_grad():
		...
		for trajectory, meta, scene in val_loader:
			...
			cnt += 1
			print(f'{cnt}/{len(val_loader)}')

			# Get scene image and apply semantic segmentation
			scene_image = val_images[scene].to(device).unsqueeze(0)
			axes[0,0].imshow(scene_image[0,:].cpu().permute(1,2,0).numpy()[:,:,::-1])
			scene_image = model.segmentation(scene_image)
			...
			for i in range(0, len(trajectory), batch_size):
				...
				[ax.cla() for ax in axes.ravel()[1:]]
				axes[0,1].imshow(observed_map[0,-1,:].cpu(), cmap='gray')
				...
				axes[1,0].imshow(pred_waypoint_map_sigmoid[0,-1,:].cpu(), cmap='gray')
				...
				axes[1,0].plot(goal_samples.squeeze(1).squeeze(1).cpu()[:,0], goal_samples.squeeze(1).squeeze(1).cpu()[:,1], 'yx')
				...
				axes[1,1].imshow(pred_waypoint_map_sigmoid[0,-1,:].cpu(), cmap='gray')
				for tr in future_samples:
					axes[1,1].plot(tr.squeeze(0).cpu()[:,0], tr.squeeze(0).cpu()[:,1])

				future_samples = torch.stack(future_samples)

				gt_goal = gt_future[:, -1:]

				axes[1,0].plot(observed[:,0], observed[:,1], 'b.')
				axes[1,1].plot(observed[:,0], observed[:,1], 'b.')
				axes[1,0].plot(gt_goal.squeeze(0).cpu()[0,0], gt_goal.squeeze(0).cpu()[0,1], 'rp')
				axes[1,1].plot(gt_goal.squeeze(0).cpu()[0,0], gt_goal.squeeze(0).cpu()[0,1], 'rp')
				plt.pause(0.1)
				input()
