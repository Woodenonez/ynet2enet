...
from test import evaluate, visualize
# from train_recursive import train
# from test_recursive import evaluate, visualize
...
class YNetTorch(nn.Module):
	def __init__(self, obs_len:int, pred_len:int, segmentation_model_fp:str, use_features_only:bool=False,
				 semantic_classes:int=6, encoder_channels:list=[], decoder_channels:list=[], waypoints:int=1):
		"""
...
class YNet:
	...
	def train(self, train_data, val_data, params, train_image_path, val_image_path, experiment_name, batch_size=8, num_goals=20, num_traj=1, device=None, dataset_name=None):
		...
		df_train, train_images = augment_data(train_data, image_path=train_image_path, image_file=img_fname, seg_mask=seg_mask)

		# Load val scene images
		val_images = create_images_dict(val_data, image_path=val_image_path, image_file=img_fname)

		# Initialize dataloaders
		train_dataset = SceneDataset(df_train, resize=params['resize'], total_len=total_len)
		train_loader = DataLoader(train_dataset, batch_size=1, collate_fn=scene_collate, shuffle=True) # each batch here is a scene
		...
		criterion = loss_func.loss_nlltr
		...
		for ep in range(params['num_epochs']):
			train_ADE, train_FDE, train_loss = train(model, train_loader, train_images, ep, self.obs_len, self.pred_len,
	...
	def visualize(self, data, params, image_path, batch_size=8, num_goals=20, num_traj=1, rounds=1, device=None, dataset_name=None):
		"""
		Val function
		:param data: pd.df, val data
		:param params: dictionary with training hyperparameters
		...
		print('Start testing')
		visualize(model, test_loader, test_images, num_goals, num_traj,
					obs_len=obs_len, batch_size=batch_size,
					device=device, input_template=input_template,
					waypoints=params['waypoints'], resize=params['resize'],
					temperature=params['temperature'], use_TTST=True,
